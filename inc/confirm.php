...
if (isset($_GET['status'])) {
    ...
    if ($_GET['status'] === 'cancelled') {
        $status = $_GET['status'];
        $update_order_status_failed = "UPDATE bob_orders SET status = 'Failed' WHERE order_id = ?";
        ...
        if ($res->status === 'success') {
            ...
            if ($amountPaid >= $amountToPay) {
                ...
                $update_payment_status = "UPDATE bob_orders SET status = 'Successful' WHERE order_id = ?";
                $stmt = mysqli_stmt_init($conn);
                ...
                if (mysqli_stmt_execute($stmt)) {
                    ...
                    echo "<script>
                            localStorage.removeItem('bwb_cart');
                          </script>";
